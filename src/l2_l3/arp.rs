...
use crate::l2_l3::ip::{Ipv4Config, Ipv4Packet};
...
impl ArpPacket {
    ...
    pub fn read(&mut self, packet: &Vec<u8>) -> Result<bool> {
        anyhow::ensure!(
            packet.len() >= ARP_LENGTH,
            "ARP should be {} bytes or more. packet.len()={}",
            ARP_LENGTH,
            packet.len()
        );
    ...
    pub fn validate(&mut self) -> Result<bool> {
        ...
        if self.hw_type != 0x1 as u16 || (self.opcode != 0x1 as u16 && self.opcode != 0x2 as u16) {
...
impl Display for ArpEntry {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        write!(
            f,
            "mac: {} creation_time: {:?} ttl: {:?}",
            self.mac, self.creation_time, self.ttl
        )
    }
}
...
impl L2Stack {
    ...
    fn receive_thread(&self) -> Result<()> {
        ...
        loop {
            ...
            if dst_mac != self.interface_mac && dst_mac != MacAddress::broadcast() {
                if LOG_PKT_DUMP {
                    log::trace!(
                        "Discarding packet. Interface mac is {}, but packet dst is to {}.",
                        self.interface_mac,
                        dst_mac
                    );
                }
    ...
    pub fn send(&self, mut ipv4_packet: Ipv4Packet, dst_mac: Option<MacAddress>) -> Result<()> {
            ...
            )
        {
            ...
            return Err(L2Error::NoGatewayError {
                target_ip: dst_ip_addr,
                l2_ip: self.interface_ipv4.address,
                l2_netmask: self.interface_ipv4.netmask,
            }
            .into());
        }
        let mut ethernet_packet = EthernetPacket::new();
        ...
        if let Some(dst) = dst_mac {
            ...
            ethernet_packet.dst = self
                .lookup_arp(&Ipv4Addr::from(ipv4_packet.dst_addr))?
                .as_bytes()
                .try_into()?;
        }
    ...
    fn resolve_arp(&self, target_ip: &Ipv4Addr) -> Result<ArpEntry> {
        ...
        while count < ARP_RETRY {
            ...
            let arp_entry = { self.arp_table.lock().unwrap().get(target_ip).cloned() };
    ...
    fn arp_handler(&self, ethernet: EthernetPacket, arp: ArpPacket) -> Result<()> {
            ...
            )
        {
            ...
            if !arp_src_mac.is_nil()
                && !arp_src_mac.is_broadcast()
                && !arp_src_mac.is_multicast()
                && !arp_src_ip.is_broadcast()
                && !arp_src_ip.is_multicast()
                && !arp_src_ip.is_link_local()
                && !arp_src_ip.is_loopback()
                && arp_dst_ip == self.interface_ipv4.address
            {
                let mut arp_table = self.arp_table.lock().unwrap();
                ...
                log::debug!(
                    "Arp entry added from request. ip: {} {}",
                    arp_src_ip,
                    arp_entry
                );
            ...
            log::debug!(
                "Replying to arp request. dst mac: {} dst ip: {}",
                arp_src_mac,
                arp_src_ip
            );
            let send_lock = self.send_channel.lock().unwrap();
            send_lock
                .send(ethernet_packet.create_packet()?.into_boxed_slice())
                .context("Failed to send reply arp packet.")?;
        } else if arp.opcode == 0x2
            && is_netmask_range(
                &self.interface_ipv4.address,
                self.interface_ipv4.netmask,
                &arp_dst_ip,
...
mod arp_tests {
    ...
    fn test_arp_packet_read(
        ...
        let result = arp_packet
            .read(&packet_data)
            .expect("Failed to read ARP packet");
        let recreated_packet = arp_packet
            .create_packet()
    ...
    fn test_arp_packet_read_error(#[case] encoded_packet: &str) {
        let packet_data = decode(encoded_packet).expect("Failed to decode hex string");
        ...
        assert!(
            result.is_err(),
            "Expected an error for incorrect packet length"
        );
    }
